...
def analyze_growth_potential(text):
    """
    Анализирует содержимое секции отчета и определяет потенциал роста продаж и бизнеса
    на основе выявленных возможностей для улучшения.
    
    Args:
        text (str): Текст секции отчета
        
    Returns:
        float: Оценка потенциала роста от 0 до 100
    """
    # Базовая оценка потенциала
    growth_score = 0
    
    # Ключевые индикаторы роста продаж (максимальный вес)
    critical_growth_phrases = [
        "увеличение продаж", "рост выручки", "расширение аудитории", "новые сегменты", 
        "повышение лояльности", "новые рынки", "кросс-продажи", "повторные продажи",
        "увеличение среднего чека", "увеличение конверсии", "рост маржинальности", 
        "снижение оттока клиентов", "повышение LTV", "рост доли рынка",
        "повышение рентабельности", "увеличение маржи", "высокая маржа", 
        "расширение ассортимента", "дополнительные услуги", "премиум-сегмент",
        "привлечение новых клиентов", "монетизация", "рост прибыли", "масштабирование бизнеса",
        "экспорт", "международные рынки", "инновационные решения", "высокий спрос",
        "тренд роста", "набирает популярность", "востребованность", "высокие рейтинги",
        "вирусный потенциал", "сарафанное радио", "органическое продвижение"
    ]
    
    # Двойственные термины, требующие проверки контекста (могут быть упомянуты как в позитивном, так и в негативном контексте)
    ambiguous_terms = [
        "скидк", "акци", "спецпредложен", "распродаж", "бонус", "программа лояльности", 
        "купон", "промокод", "подарок", "бесплатная доставка", "кешбэк", "бонусные баллы",
        "персонализац", "отзыв", "рекомендац", "дополнительн", "гарантия", "сервис",
        "обслуживание", "доставка", "гибк", "цен", "стоимост", "удобств", "комфорт"
    ]
    
    # Негативные контексты, которые снижают значимость упоминания двойственных терминов
    negative_contexts = [
        "отсутств", "не хватает", "не предоставл", "не предлага", "недостаточн", 
        "не работает", "не действует", "плох", "некачественн", "редк", "маленьк", "скудн",
        "не дают", "не работал", "не распространя", "не предусмотр", "у конкурентов", 
        "в других магазинах", "в других местах", "нет", "минимальн", "слабая", "завышен",
        "обещали", "но не дали", "отказ", "завышенный", "не оправдывает", "жалоб", 
        "недоволь", "разочарова", "обеща", "не получил", "не соответств", "ложн", "обман"
    ]
    
    # Фразы высокого потенциала (высокий вес)
    high_impact_phrases = [
        "конкурентное преимущество", "уникальное торговое предложение", "лидер мнений", 
        "целевая аудитория", "лояльная база", "сильные стороны товара", "инновация",
        "тренд", "растущий рынок", "brand awareness", "узнаваемость бренда", 
        "положительные отзывы", "престижность", "премиальный сегмент", "клиентский опыт",
        "удобство использования", "простота", "удовлетворенность клиентов", "превосходит ожидания",
        "высокое качество", "надежность", "эффективность", "многофункциональность", 
        "разнообразие применения", "гибкость настроек", "адаптивность", "персонализация",
        "модульность", "масштабируемость", "экологичность", "органический", "натуральный состав",
        "инновационная технология", "революционный подход", "прорыв в отрасли"
    ]
    
    # Фразы среднего потенциала (средний вес)
    medium_impact_phrases = [
        "улучшение дизайна", "расширенная функциональность", "эргономичность", "комфорт", 
        "долговечность", "экономичность", "энергоэффективность", "актуальный дизайн",
        "современное решение", "удобная упаковка", "практичность", "эстетичность",
        "модный", "стильный", "трендовый", "популярный", "универсальность", "совместимость",
        "интеграция", "техподдержка", "консультации", "обучающие материалы",
        "сезонные предложения", "дополнительные аксессуары", "расходные материалы"
    ]
    
    # Находим процентные показатели роста в тексте
    percentage_pattern = r'увеличени[еюя]|рост|повышени[еюя]|улучшени[еюя].*?(\d+(?:\.\d+)?)%'
    percentages = re.findall(percentage_pattern, text.lower(), re.IGNORECASE)
    percentages = [float(p) for p in percentages if float(p) <= 100]
    
    # Особые паттерны, указывающие на возможности
    opportunity_pattern = r'возможност|потенциал|перспектив|шанс|развити[еюя]|усоверш|улучш|оптимиз|модерниз|обнов|усилен|укреплен|продвижен|рекламн|маркетинг|позиционир|целев[ая][ую] аудитор|сегмент|ниш|рыноч'
    has_opportunity = bool(re.search(opportunity_pattern, text.lower(), re.IGNORECASE))
    
    # Паттерны, указывающие на конкурентное преимущество
    advantage_pattern = r'преимуществ|отличи|уникальн|эксклюзивн|особенност|выделя[еютс]|конкурентн|лучш[еий]|превосход|лидирующ|топов|флагман|премиум|премиальн|перв[ыоеий] в|передов|выдающи|высш|качественн'
    has_advantage = bool(re.search(advantage_pattern, text.lower(), re.IGNORECASE))
    
    # Целевые аудитории и сегменты, которые указывают на рост
    target_audiences = [
        "молодежь", "миллениалы", "поколение z", "gen z", "поколение y", "молодые семьи", 
        "мамы", "родители", "дети", "подростки", "студенты", "профессионалы", 
        "бизнес-пользователи", "офисные работники", "предприниматели", "малый бизнес", 
        "средний бизнес", "геймеры", "спортсмены", "активный образ жизни", "женщины", 
        "мужчины", "старшее поколение", "пенсионеры", "высокодоходная аудитория", 
        "средний класс", "массовый сегмент", "люкс-сегмент", "премиум-сегмент", 
        "корпоративные клиенты", "оптовики", "дропшиппинг", "маркетплейсы"
    ]
    has_target_audience = any(audience.lower() in text.lower() for audience in target_audiences)
    
    # Проверка на общее позитивное восприятие
    positive_sentiment = r'хорош|отличн|прекрасн|великолепн|качественн|надежн|прочн|долговечн|удобн|комфорт|красив|эстетич|приятн|впечатля[еюящ]|радуе|восхища|довол[еь]н|подходящ|идеаль|достойн|лучш|высок[оа][ем]|премиаль|инновацион|передов|нов|современ|максимал|оптимал|популяр|востребован|рекомен|мечт|ждали|превосходит'
    has_positive = bool(re.search(positive_sentiment, text.lower(), re.IGNORECASE))
    
    # Анализ рекомендаций для улучшения продукта
    improvement_pattern = r'рекоменд[ауюе]|предлага[еютюя]|совету[еюя]|стоит|следует|необходим[оаыи]|важно|улучш[еиать]|оптимизир[оуюеват]|внедр[еиять]|добав[иьлят]|измен[ияеть]'
    has_recommendations = bool(re.search(improvement_pattern, text.lower(), re.IGNORECASE))
    
    # Подсчет упоминаний ключевых фраз роста
    critical_count = sum(1 for phrase in critical_growth_phrases if re.search(r'\b' + re.escape(phrase) + r'\b', text.lower()))
    high_impact_count = sum(1 for phrase in high_impact_phrases if re.search(r'\b' + re.escape(phrase) + r'\b', text.lower()))
    medium_impact_count = sum(1 for phrase in medium_impact_phrases if re.search(r'\b' + re.escape(phrase) + r'\b', text.lower()))
    
    # Проверка двойственных терминов на наличие негативного контекста
    positive_ambiguous_count = 0
    for term in ambiguous_terms:
        # Проверяем каждое вхождение термина
        term_positions = [m.start() for m in re.finditer(term, text.lower())]
        for pos in term_positions:
            # Получаем контекст вокруг термина (30 символов до и после)
            start = max(0, pos - 30)
            end = min(len(text), pos + 30)
            context = text.lower()[start:end]
            
            # Проверяем, есть ли негативный контекст вокруг термина
            has_negative_context = any(neg in context for neg in negative_contexts)
            
            # Если негативного контекста нет, увеличиваем счетчик позитивных упоминаний
            if not has_negative_context:
                positive_ambiguous_count += 1
    
    # Расчет базовой оценки на основе найденных фраз
    growth_score += critical_count * 15  # Максимальный вес
    growth_score += high_impact_count * 8  # Высокий вес
    growth_score += medium_impact_count * 4  # Средний вес
    growth_score += positive_ambiguous_count * 3  # Средний вес для двойственных терминов в позитивном контексте
    
    # Анализ процентных показателей роста
    if percentages:
        max_percentage = max(percentages)
        if max_percentage > 50:  # Если потенциал роста более 50%
            growth_score += 25
        elif max_percentage > 30:  # Если потенциал роста более 30%
            growth_score += 15
        elif max_percentage > 10:  # Если потенциал роста более 10%
            growth_score += 5
    
    # Повышаем оценку для перспективных возможностей
    if has_opportunity:
        growth_score += 15
    
    # Повышаем оценку для конкурентных преимуществ
    if has_advantage:
        growth_score += 20
    
    # Повышаем для целевых аудиторий
    if has_target_audience:
        growth_score += 15
    
    # Повышаем при наличии общего позитивного восприятия
    if has_positive:
        growth_score += 10
    
    # Повышаем при наличии конкретных рекомендаций по улучшению
...
def prioritize_pdf_sections(responses_dir, pdf_dir):
    ...
    на основе двух критериев: 1) насколько сильно проблемы влияют на бизнес и 
    ...
    # Анализируем каждый файл ответа и определяем его критичность и потенциал роста
    for response_file in response_files:
        ...
        try:
            ...
            # Анализируем влияние проблем на бизнес
            impact_score = analyze_business_impact(content)
            
            # Анализируем потенциал роста продаж
            growth_score = analyze_growth_potential(content)
            
            # Рассчитываем комбинированную оценку
            # Можно настроить веса для каждой метрики в зависимости от приоритетов бизнеса
            impact_weight = 0.5  # Вес для критичности проблем (50%)
            growth_weight = 0.5  # Вес для потенциала роста (50%)
            combined_score = (impact_score * impact_weight) + (growth_score * growth_weight)
            
            # Сохраняем оценки и путь к файлу
            section_scores[pdf_file] = {
                'impact': impact_score,
                'growth': growth_score,
            ...
            print(f"Секция {section_num}: Критичность проблем = {impact_score:.1f}/100, "
                  f"Потенциал роста = {growth_score:.1f}/100, "
                  f"Итоговая оценка = {combined_score:.1f}/100")
                  
        except Exception as e:
            print(f"Ошибка при анализе файла {response_file}: {e}")
    
    # Сортируем PDF-файлы по убыванию комбинированной оценки
    # При равных оценках сохраняем исходный порядок секций
    sorted_files = sorted(
        section_scores.keys(),
        key=lambda x: (-section_scores[x]['combined'], section_scores[x]['section'])
    )
    
    # Выводим подробную информацию о результатах анализа
    print("\nДетальные результаты анализа секций (от наиболее до наименее приоритетных):")
    for i, pdf_file in enumerate(sorted_files):
        score = section_scores[pdf_file]
        section_num = score['section']
        print(f"{i+1}. Секция {section_num}: Критичность = {score['impact']:.1f}, "
              f"Потенциал роста = {score['growth']:.1f}, "
              f"Итоговая оценка = {score['combined']:.1f}")
